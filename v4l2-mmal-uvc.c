...
static const struct uvc_frame_info uvc_frames_rgb24[] = {
  {640, 360, {66666, 100000, 500000, 0},},
//  {736, 480, {500000, 0},},
  {1280, 720, {  400000, 0},},
  {0, 0, {0,},},
...
static const struct uvc_format_info uvc_formats[] = {
  ...
  {V4L2_PIX_FMT_RGB24, uvc_frames_rgb24},
};
...
time_me (char *fn_name, struct v4l2_device *dev)
{
  ...
  if(i)
    ...
    ft += ((float)(stt.tv_nsec - dev->last.tv_nsec)/1e9 + (float)(stt.tv_sec - dev->last.tv_sec) );
  if(i%20==0)
    printf ("%s %f s\n", fn_name, ft/i );
  dev->last = stt;
  i++;
...
v4l2_reqbufs_mmap (struct v4l2_device *dev, int nbufs)
{
  ...
  if (ret < 0) {
    if (ret == -EINVAL)
      ...
      printf ("V4L2: mmap VIDIOC_REQBUFS error %s (%d) %d.\n",
...
v4l2_reqbufs (struct v4l2_device *dev, int nbufs)
{
  ...
  switch (dev->memtype) {
  case V4L2_MEMORY_MMAP:
    ret = v4l2_reqbufs_mmap (dev, nbufs);	// should be used
    break;
...
int video_querycap (struct v4l2_device *dev, unsigned int *capabilities)
{ 
		     ...
		     unsigned int offset, unsigned int padding)
{
  ...
  for (i = 0; i < rb.count; ++i) {
    ...
    case V4L2_MEMORY_USERPTR:
      ret =
	video_buffer_alloc_userptr (dev, &buffers[i], &buf, offset, padding);
      break;
    ...
    if (!buffer_export
	(dev->v4l2_fd, dev->type, i, &buffers[i].dma_fd,
	 &buffers[i].vcsm_handle)) {
	...
	return -1;
      ...
      if (dev->can_zero_copy)
	mmal_buf->data =
	  (uint8_t *) vcsm_vc_hdl_from_hdl (buffers[i].vcsm_handle);
      else
	mmal_buf->data = buffers[i].mem[0];
      mmal_buf->alloc_size = buf.length;
      buffers[i].mmal = mmal_buf;
      print
	("Linking V4L2 buffer index %d ptr %p to MMAL header %p. mmal->data 0x%X\n",
...
static int
video_prepare_capture (struct v4l2_device *dev, int nbufs,
...
static int
video_queue_all_buffers (struct v4l2_device *dev, enum buffer_fill_mode fill)
{
  unsigned int i;
  int ret;
  print ("video_queue_all_buffers try %d \n", dev->nbufs);
  /* Queue the buffers. */
  for (i = 0; i < dev->nbufs; ++i) {
...
static int
v4l2_open (struct v4l2_device **v4l2, char *devname,
...
uvc_video_set_format (struct uvc_device *dev)
{
  ...
  if (ret < 0) {
    printf ("UVC: Unable to set V4L2_PIX_FMT_MJPEG format %s (%d).\n",
	    strerror (errno), errno);
    return ret;
  }
  printf ("UVC: dev %d %d\n", dev->width, dev->height);
  printf ("UVC: uvc_video_set_format Setting format to: %c%c%c%c %ux%u\n",
...
uvc_video_process (struct uvc_device *dev)
{
  ...
  if (ret < 0) {
    printf ("UVC: unable to dequeue ubuf %s (%d)\n", strerror (errno), errno);
    return ret;
  }
  else {
...
uvc_video_reqbufs_userptr (struct uvc_device *dev, int nbufs)
{
  ...
  if (ret < 0) {
    ...
    printf ("UVC uvc_video_reqbufs_userptr okay\n");
  }
...
//should we queue empty buffers for first time?
			    ...
			    int iframe, int iformat)
{
  ...
  case V4L2_PIX_FMT_MJPEG:
    ...
    break;
   case V4L2_PIX_FMT_RGB24:
    ctrl->dwMaxVideoFrameSize = frame->width * frame->height * 3;
    break;
 }
  printf("bmHint %d, bFormatIndex %d, bFrameIndex %d, dwFrameInterval %d, dwMaxVideoFramSize %d\n",
  ctrl->bmHint, ctrl->bFormatIndex, ctrl->bFrameIndex, ctrl->dwFrameInterval, ctrl->dwMaxVideoFrameSize);
  ...
  ctrl->bmFramingInfo = 3;// 0-no changewei
			 ...
			 struct uvc_request_data *data)
{
  ...
  switch (format->fcc) {
  case V4L2_PIX_FMT_YUYV:
    printf ("UVC: event proc UYVY frame->width %d frame->height %d %d\n",
	    frame->width, frame->height, iformat);
  ...
  case V4L2_PIX_FMT_MJPEG:
    ...
    break;
  case V4L2_PIX_FMT_RGB24:
    printf ("UVC: event proc RGB24 frame->width %d frame->height %d %d\n",
	    frame->width, frame->height, iformat);
    target->dwMaxVideoFrameSize = frame->width * frame->height * 3;
    // uses frame size and then dev->width & height get changed
    break;
  }
...
uvc_events_process (struct uvc_device *dev)
{
  ...
  case UVC_EVENT_STREAMON:
    printf("UVC_EVENT_STREAMON bulk %d\n",dev->bulk);
    if (!dev->bulk)
...
uvc_events_init (struct uvc_device *dev)
{
  ...
  case V4L2_PIX_FMT_MJPEG:
    ...
    break;
  case V4L2_PIX_FMT_RGB24:
    payload_size = dev->width * dev->height * 3;
    break;
  }

  printf ("uvc; events init udev->width %d, udev->height %d %d \n", dev->width,
	  dev->height, dev->probe.dwMaxPayloadTransferSize);
...
save_thread (void *arg)
{
  ...
  while (!dev->thread_quit) {
    ...
    print
      ("Buffer %p saving, %p, alloc %d, length %d, timestamp %llu, flags %04X\n, index %d, \n",
       buffer, buffer->data, buffer->alloc_size, buffer->length,  buffer->pts, buffer->flags,
       dev->buffers->idx );
    ...
    ubuf.length = buffer->length;//buffer->alloc_size;
    //ubuf.index = dev->buffers->idx  ;//dev->vbuf.index;;	//vbuf.index;L
    ubuf.index = dev->vbuf.index;;	//vbuf.index;L
    ubuf.bytesused = buffer->alloc_size; //buffer->length;
    
//  if(dev->dq_ubuf_ok==false){  
    ret = ioctl (dev->udev->uvc_fd, VIDIOC_QBUF, &ubuf);
    if (ret < 0) {
      print("UVC: Unable to queue buffer %d: %s (%d) %lx.\n",
	     dev->vbuf.index, strerror (errno), errno, ubuf.m.userptr);
...
isp_input_callback (MMAL_PORT_T * port, MMAL_BUFFER_HEADER_T * buffer)
{
  ...
  unsigned int i;
  //print("isp_input_callback Buffer %p (->data %p) returned\n", buffer, buffer->data);
  for (i = 0; i < dev->nbufs; i++) {
...
isp_output_callback (MMAL_PORT_T * port, MMAL_BUFFER_HEADER_T * buffer)
{
  ...
  //if (buffer->flags & MMAL_BUFFER_HEADER_FLAG_FRAME_END) {
    /*printf ("buf details %p, %p, %d %d %d %d %d\n", buffer, buffer->data,
       dev->buffers[0].idx, buffer->alloc_size,
       dev->vbuf.index, buffer->length, port->is_enabled); */
       //time_me("isp",dev);
    if (port->is_enabled) {
      dev->counter++;
...
enable_mmal_input (struct v4l2_device *dev)
{
  ...
  if (mmal_port_parameter_set_boolean
      (dev->isp->input[0], MMAL_PARAMETER_ZERO_COPY,
       dev->can_zero_copy) != MMAL_SUCCESS) {
...
// init_mmal
static int
init_mmal (struct v4l2_device *dev, int nbufs)
{
  ...
  print ("init mmal get format\n");
  ret = ioctl (dev->v4l2_fd, VIDIOC_G_FMT, &fmt);
  if (ret < 0) {
    print ("Unable to get format: %s (%d) %d.\n", strerror (errno), errno,
	   dev->v4l2_fd);
    return ret;
  }
  ...
  port->format->es->video.width = VCOS_ALIGN_UP (fmt.fmt.pix.width, 32);
  port->format->es->video.height = VCOS_ALIGN_UP (fmt.fmt.pix.height, 16);
...
//cropping
  ...
  unsigned int mmal_stride =
    mmal_encoding_width_to_stride (info->mmal_encoding,
				   port->format->es->video.width);
  if (mmal_stride != fmt.fmt.pix.bytesperline) {
    if (video_set_format
	(dev, fmt.fmt.pix.width, fmt.fmt.pix.height,
	 fmt.fmt.pix.pixelformat, mmal_stride, fmt.fmt.pix.sizeimage,
	 fmt.fmt.pix.field, fmt.fmt.pix.flags) < 0)
  ...
  print ("Created pool of %d buffers, size %d for isp_in\n", nbufs, 0);
...
// isp_output setup
  ...
  //isp_port->format->encoding = dev->dst_mmal_enc;     //setup encoding format
  isp_output->format->encoding = MMAL_ENCODING_YUYV;	//setup encoding format
  ...
  isp_output->buffer_num = 8;//8 is a good number larger the number less tearing on output
  //isp_output->buffer_size = isp_output->buffer_size_recommended;


  status = mmal_port_format_commit (isp_output);
  if (status != MMAL_SUCCESS) {
    print ("ISP o/p commit fail, MMAL_ENCODING \n");	//, dev->dst_mmal_enc);
    return -1;
  }

  /*don't use otherwise buffer->data in callback cannot be accessed
  status = mmal_port_parameter_set_boolean (isp_output, MMAL_PARAMETER_ZERO_COPY, MMAL_TRUE); */
 
 isp_output->userdata = (struct MMAL_PORT_USERDATA_T *) dev;	// user data has copyof dev

  // create buffers and headers and memory buffers using pool API

  print ("Created pool of %d buffers of size %d for isp_out\n",
	 isp_output->buffer_num, isp_output->buffer_size);
  dev->output_pool =
    mmal_port_pool_create (isp_output, isp_output->buffer_num,
  ...
  // queue for the processed frames
  ...
  vcos_status =
    vcos_thread_create (&dev->save_thread, "save-thread", NULL, save_thread,
			dev);
  ...
  if (status != MMAL_SUCCESS) {
    print ("isp_output_callback failed\n");
    return -1;
  }

  // send empty buffers to the output port of isp to allow isp start make frames
  // do for each buffer in queue
  unsigned int i;
  for (i = 0; i < isp_output->buffer_num; i++) {
...
main (int argc, char *argv[])
{
  ...
  int vnbufs = 2;		//isp input and output buffers
...
/* video_init */
  ...
  case 1:
    ...
    break;
  case 2:
    udev->fcc = V4L2_PIX_FMT_RGB24;
    udev->imgsize = udev->width * udev->height * 3;
    break;
  case 0:
  default:
  ...
  video_set_format (vdev, 1280, 720, V4L2_PIX_FMT_UYVY , 0, 0, field, fmt_flags);// eos m5 ok
 // video_set_format (vdev, 1280, 720, V4L2_PIX_FMT_RGB24, 0, 0, field, fmt_flags);

  video_set_dv_timings (vdev);
  video_get_format (vdev);
  init_mmal (vdev, vnbufs);
  video_prepare_capture (vdev, vnbufs, 0, "", BUFFER_FILL_NONE);
